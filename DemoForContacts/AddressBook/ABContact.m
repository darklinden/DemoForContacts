...
- (UIImage *) image
{
    ...
    if (!imageData)
    {
        imageData = ABPersonCopyImageDataWithFormat(record, kABPersonImageFormatThumbnail);
        if (!imageData) {
            return nil;
        }
    }
...
- (NSArray *) labelsForProperty: (ABPropertyID) anID
{
    ...
    for (int i = 0; i < ABMultiValueGetCount(theProperty); i++)
    {
        ...
        NSString *value = (__bridge_transfer NSString *)ABMultiValueCopyValueAtIndex(theProperty, i);
        if(label)
        {
        ...
        {
            if (value) {
                [labels addObject:(__bridge NSString *)kABHomeLabel];
            }
        }
    }
...
- (NSData *)baseVCardData
{
    ABContact *tmp_contact = [ABContact contact];
    tmp_contact.firstname = self.firstname;
    tmp_contact.lastname = self.lastname;
    tmp_contact.middlename = self.middlename;
    
    NSArray *array_phone = self.phoneDictionaries;
    for (NSDictionary *dict in array_phone) {
        [tmp_contact addPhone:dict];
    }
    
    NSArray *array_email = self.emailDictionaries;
    for (NSDictionary *dict in array_email) {
        [tmp_contact addEmail:dict];
    }
    
    CFMutableArrayRef cfarray = CFArrayCreateMutable(CFAllocatorGetDefault(), 1, NULL);
    CFArrayAppendValue(cfarray, tmp_contact.record);
    CFDataRef data = ABPersonCreateVCardRepresentationWithPeople(cfarray);
    CFRelease(cfarray);
...
{
    CFMutableArrayRef cfarray = CFArrayCreateMutable(CFAllocatorGetDefault(), 1, NULL);
    CFArrayAppendValue(cfarray, self.record);
    CFDataRef data = ABPersonCreateVCardRepresentationWithPeople(cfarray);
    CFRelease(cfarray);
    return (__bridge NSData *)data;
}
...
- (NSString *)description
{
    ...
    if (self.phoneArray.count) {
        NSString *phone = self.phoneArray[0];
        if (phone.length) {
            return phone;
        }
    }
    
    if (self.emailArray.count) {
        NSString *email = self.emailArray[0];
        if (email.length) {
            return email;
        }
    }
...
{
    NSMutableArray *pArr_ret = [NSMutableArray array];
    NSArray *pArr_dict = self.addressDictionaries;
    for (int i = 0; i < pArr_dict.count; i++) {
        NSDictionary *pDict_addr = pArr_dict[i];
        NSString *pStr_addr = ABCreateStringWithAddressDictionary(pDict_addr, YES);
        if (pStr_addr) {
            [pArr_ret addObject:pStr_addr];
        }
    }
    return [NSArray arrayWithArray:pArr_ret];
}
